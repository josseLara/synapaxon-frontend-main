...
const MyCreatedQuestionsPage = () => {
    ...
    const fetchCreatedQuestions = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_URL || 'https://synapaxon-backend.onrender.com'}/api/questions?createdBy=me&page=${pagination.current}&limit=${pagination.limit}`,
          {
            headers: {
  ...
  const handleRequestEdit = async (questionId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'https://synapaxon-backend.onrender.com'}/api/questions/${questionId}/request-edit`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleRequestDelete = async (questionId) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'https://synapaxon-backend.onrender.com'}/api/questions/${questionId}/request-delete`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
