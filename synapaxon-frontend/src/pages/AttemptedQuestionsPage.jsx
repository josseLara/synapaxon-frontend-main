...
const AttemptedQuestionsPage = () => {
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true);
      let url = `${import.meta.env.VITE_API_URL || 'https://synapaxon-backend.onrender.com'}/api/student-questions/history?page=${pagination.current}&limit=${pagination.limit}`;
      ...
      const detailsPromises = data.data.map(async (q) => {
        try {
          const detailRes = await fetch(
            `${import.meta.env.VITE_API_URL || 'https://synapaxon-backend.onrender.com'}/api/questions/${q.question?._id || q._id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
          return detailRes.ok
